...
GaussianDoG::GaussianDoG(int kernel_size, double variance) {
    ...
    for (int x = -half_size; x <= half_size; x++) {
        ...
        coef_1st.at<float>(x + half_size) = gaussian1st(-x, variance);
        coef_dog.at<float>(x + half_size) = gaussian0th(x, variance * 0.5) - gaussian0th(x, variance);
    }
    ...
    m_ClProgramUnsigned = m_ClContext.getProg(program_source, cv::format("-D N=%d -D T=uchar", kernel_size), error_message);
    if (!error_message.empty()) {
        CV_Error(cv::Error::OpenCLApiCallError, error_message.c_str());
    }
    m_ClProgramSigned = m_ClContext.getProg(program_source, cv::format("-D N=%d -D T=char", kernel_size), error_message);
    if (!error_message.empty()) {
        CV_Error(cv::Error::OpenCLApiCallError, error_message.c_str());
    }
}
...
void GaussianDoG::compute(const cv::Mat &src) {
    ...
    */
    ...
    // ブラーを掛ける
    cv::ocl::Kernel blur_x_kernel("unsignedConvoluteX", m_ClProgramUnsigned);
    cv::ocl::Kernel blur_y_kernel("unsignedConvoluteY", m_ClProgramUnsigned);
    ...
    blur_x_kernel.run(2, global_size, local_size, true);
    ...
    blur_y_kernel.run(2, global_size, local_size, true);

    // 正規分布関数の一次微分で畳み込む
    cv::ocl::Kernel derivative_x_kernel("signedConvoluteX", m_ClProgramUnsigned);
    cv::ocl::Kernel derivative_y_kernel("signedConvoluteY", m_ClProgramUnsigned);
    ...
    derivative_x_kernel.run(2, global_size, local_size, true);
    ...
    derivative_y_kernel.run(2, global_size, local_size, true);

    // DoGを畳み込む
    cv::ocl::Kernel dog_x_kernel("signedConvoluteX", m_ClProgramUnsigned);
    cv::ocl::Kernel dog_y_kernel("signedConvoluteY", m_ClProgramUnsigned);
    ...
    dog_x_kernel.run(2, global_size, local_size, true);
